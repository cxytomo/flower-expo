...
//window on scroll, nav gives the hint of where you are.
var box = init();
...
function determ_p(){
	...
	var position
	, top;
	top = getScrollOffset().y;
	if(top >= 0 & top < box.topshowFlower) {
		position = "#header";
		return position;
	}
	if(top >= box.topshowFlower & top < box.topticket) {
		position = "#showFlower";
		return position;
	}
	if(top >= box.topticket) {
		position = "#ticket";
		return position;
	}
}
...
function init(){
	...
	toTop.topheader = $('#header')[0].getBoundingClientRect().top + offset;
	toTop.topheader = Math.floor(toTop.topheader);
	toTop.topshowFlower = $('#showFlower')[0].getBoundingClientRect().top + offset;
	toTop.topshowFlower = Math.floor(toTop.topshowFlower);
	toTop.topticket = $('#ticket')[0].getBoundingClientRect().top + offset;
	toTop.topticket = Math.floor(toTop.topticket);
	console.log(toTop);
	return toTop;
}
